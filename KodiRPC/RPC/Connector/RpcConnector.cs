...
namespace KodiRPC.RPC.Connector
{
    public class RpcConnector : IRpcConnector
    {
        ...
        public JsonRpcResponse<T> MakeRequest<T>(string rpcMethod, object parameters, string id = "KodiJSON-RPC", int timeout = 10000)
        {
                ...
                if (webResponse != null)
                {
                    switch (webResponse.StatusCode)
                    {
                        ...
                        case HttpStatusCode.Unauthorized:
                            throw new RpcException("The RPC request was rejected by the server (Unauthorized).", e);
